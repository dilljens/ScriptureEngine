...
def sim_matrix_emb(verses):
    ...
    for i in range(n):
        ...
        for j in range(i+1, n):
            ...
            if nj == 0: continue
            s = float(sum(a*b for a,b in zip(ei, ej))) / float(ni * nj)
            m[i][j] = s; m[j][i] = s
    return m
    ...
    for pivot in range(3, n-3):
        for window in range(3, min(max_w, pivot, n-pivot-1) + 1):
            ...
            for d in range(1, window+1):
                ...
                if l < 0 or r >= n: break
                sim = matrix[l][r]
                local = (baseline[l] + baseline[r]) / 2
                ctr = sim - local
                pairs.append({"left": vlist[l]["id"], "right": vlist[r]["id"], "sim": round(float(sim), 3), "ctr": round(float(ctr), 3)})

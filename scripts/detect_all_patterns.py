    ...
    for w in range(min_window, max_w + 1):
        for pv in range(w, n - w):
            ...
            for d in range(1, w+1):
                ...
                s = float(sims[left][r])
                c = s - (bl[left] + bl[r])/2
                prs.append({"l": vlist[left]["id"], "r": vlist[r]["id"], "s": round(s,3)})
                ctrs.append(c)
            if len(prs) >= 2:
...
def run_all(conn):
    ...
    for wid, bids in all_books.items():
        for bid in bids:
            ...
            for link in links: link["book"] = bid; link["work"] = wid
    ...
    for link in word_link_results[:5]:
        print(f"    {link['work']}.{link['book']}: \"{link['keyword']}\" — {link['verse_a']} ↔ {link['verse_b']} sim={link['similarity']}")
    ...
    stored_links = 0
    for link in word_link_results[:500]:
        safe_ins("patterns", "book_id, start_verse, end_verse, pattern_type, description, confidence, discovered_by, metadata",
                 (link["book"], link["verse_a"], link["verse_b"], "word_link",
                  f"\"{link['keyword']}\" links {link['verse_a']} and {link['verse_b']} (sim={link['similarity']})",
                  min(float(link["similarity"]), 1.0), "pattern_detector",
                  json.dumps({"keyword": link["keyword"], "work": link["work"], "similarity": link["similarity"]})))
        stored_links += 1

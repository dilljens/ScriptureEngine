...
- The Ascent (heavenly chain, branched): six stations × pick-1-of-2, one way per tier forever (64 heavens). Enoch's tour gives the meta-layer its spine — treasuries of winds (1 En 18.1), portals/luminaries (1 En 34, 72), crystal throne (1 En 14.18, all in-corpus) — with Genesis anchors where they live (first fruits, primordial light, Shabbat). Pairs are real tradeoffs on existing perm keys (head-start vs scaling, taps vs idle, burst vs steady, active vs offline), so no consumer changed. Sim walks the left path; branch coverage is in self-checks.
